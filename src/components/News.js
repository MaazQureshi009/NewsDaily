...
const News = (props)=> {
  ...
  const fetchMoreData = async () => {
    const url = `https://newsapi.org/v2/top-headlines?${props.country}&category=${props.category}&apiKey=5e7a1d20b95e45faa64687a58314593b&page=${page+1}&pageSize=${props.pageSize}`;
    setPage(page+1)
    let data = await fetch(url);
    let parsedData = await data.json();
    // console.log(parsedData);
    ...
    return (
      <>
      <h2 className='text-center' style={{ margin: '30px 0px', marginTop: '80px' }}>NewsDaily - Top {capitalizeFirstLetter(props.category)} Headlines</h2>
        {loading && <Spinner/>}
    <InfiniteScroll
